...
\begin{definition}
    ...
    \end{enumerate}
    A seminorm is thus almost a norm, but it's not positively definite.
\end{definition}
...
The set $U \subseteq V$ is open iff for all $x_0 \in U$ there exist seminorms $p_1, \dots, p_n \in \mathcal{P}$
...
Equivalently, for all $x \in X \setminus \{0\}$ there exists a seminorm $p \in \mathcal{P}$ such that $p(x) \neq 0$.

\begin{corollary}
  Let $X$ be a space with a topology generyted by a family of seminorms. Then $X$ is a L CS iff it is Hausdorff.
\end{corollary}

\begin{myproof}
    Start with $(\Rightarrow)$. Let $x, y \in X$ be two distinct points. There exists a seminorm $p$
    such that $p(x - y) = b \neq 0$. Then define the sets 
    $$V = U\left(x, p, \frac{b}{2}\right),\quad W = U\left(y, p, \frac{b}{2}\right)$$
    and by triangle inequality property of seminorm, $V$ and $W$ separate the points $x, y$.
    Now the converse $(\Leftarrow)$. Choose a point $X \ni x \neq 0$. Then there exist the open sets $0 \in V, x \in W$
    that separate $0$ from $X$. There exists a subbasis set $U(0, p, \varepsilon) \subseteq V$ and $p(x - 0) = p(x) \neq 0$.
\end{myproof}
...
\begin{remark}
    Weak and weak-* topology can be defined if $X$ is merely a LCS.
\end{remark}
...
\begin{definition}
  ...
  \begin{itemize}
    \item balanced if:
    $$x \in A,\ \alpha \in \F,\ |\alpha| \leq 1 \Rightarrow \alpha \cdot x \in A.$$
    \item absorbing if:
...
\begin{corollary}[The extension theorem]
  ...
  then there exists $F \in X^*$ such that $F\big|_Y = f$ and $\|F\| = \| f\|$.
\end{corollary}

\begin{corollary}[The separation theorem]
  Suppose $X$ is a LCS and $A, B \subseteq X$ are disjoint closed convex sets.
  If $B$ is compact then there exists $f \in X^*$ that seperates $A$ from $B$:
...
\begin{myproof}
  ...
  is an open neighborhhod of $0$ in $X$. Then there exist $p_1, \dots, p_r \in \mathcal{P}$
  and a $\varepsilon > 0$ such that 
...
\begin{example}
  ...
  $$\ext ((\ell^\infty)_1) = \{(\pm 1, \pm 1, \dots)\},\quad \ext ((\ell^1)_1) = \{(0, 0, \dots, \pm 1, \dots)\}.$$
\end{example}
...
\begin{example}
  ...
  \begin{enumerate}
    ...
    \item $\ext (L^1 ([0, 1]))_1 = \emptyset$;
    \item $\ext (C[0, 1])_1 = \{\pm 1\}$.
  \end{enumerate}
\end{example}

\begin{definition}
  For a vector space $X$ and $A \subseteq X$ define a convex hull 
  $$\co A = \{\sum_{i = 0}^n \alpha_i x_i\ |\ n \in \N, \alpha_i \geq 0, \sum_{i = 0} ^n \alpha_i = 1, x_i \in A\}.$$
  If $X$ is a LCS, then $\overline{\co} A = \overline{\co A}$.
\end{definition}

\begin{proposition}
  Convex hull $\co A$ is the smallest convex set that contains $A$.
  If $X$ is also a $TVS$, then $\overline{\co} A$ is the smallest closed convex set that contains $A$.
\end{proposition}
...
\begin{myproof}
  ...
  Since $C \in \mathcal{F}$, it is nonempty. The set $\mathcal{F}$ is then partially ordered and each 
  increasing chain in $\mathcal{F}$ has an upper bound (intersection).
  This tells us that we can apply Zorn's lemma so there exists a maximal element $F_0 \in \mathcal{F}$.
  We prove that $F_0 \in \ext C$. Assume that there are distinct $x, y \in F_0$.
  By Hahn-Banach, there exists an $\Phi \in X^*$ such that $\Phi(x) \neq \Phi(y)$.
