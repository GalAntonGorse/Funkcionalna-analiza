...
\begin{proposition}[Artin-Wedderburn for finite-dimensional $C^*$-algebras]
...
Given such a $\pi_f$, we define the $C^*$-algebra 
...
\begin{remark}
  The algebraic group algebra $\C [G]$ is dense in both $C_{\lambda} ^* (G)$ and $C^* (G)$.
\end{remark}
...
\begin{example}
  $\widehat{\quot{\Z}{n\Z}} = \quot{\Z}{n\Z}$, $\widehat{\R} = \R$, $\widehat{\Z} = \mathbb{T}$, $\widehat{\mathbb{T}} = {\Z}$.
\end{example}

\begin{theorem}[Pontryagin]
  If $G$ is a locally compact abelian group (its underlying topological space is locally compact Hausdorff),
