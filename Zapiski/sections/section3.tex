...
\begin{example}
  If $\dim \mathcal{H} = \infty$, then $\quot{\bh}{\mathcal{K}(\mathcal{H})}$ is a so-called \emph{Calkin algebra}.
  Calkin algebra is simple and it does not have a separable representation. We will prove this later, when we have more tools at our disposal.
\end{example}
...
\footnote{The reader should consult Misamichi Takesaki, \emph{Theory of Operator Algebras I}, Theorem I.10.2}
...
\begin{myproof}
  ...
  \begin{enumerate}
      ...
      there must exist some $x_2 \in V_1$, such that $x_2 \neq x$. By Hausdorff property, there must exist open disjoint neighborhoods 
...
Let $p_i \in C$ be an element that corresponds to characteristic function $\chi_{\{\omega_i\}} \in C(\sigma(C))$.
...
where each of $A p_i$ has trivial center. By previous lemma, $A p_i$
is a simple finite-dimensional $C^*$-algebra. So it suffices to prove the structure theorem for a simple finite-dimensional $C^*$-algebras.
...
\end{myproof}
  ...
  In operator algebras, we usually consider topological groups that are locally compact and Hausdorff.
  For such groups, there exists a so-called \emph{Haar measure} $\mu$. This measure allows us to 
  consider the $C^*$-algebras $L^2 (G)$, $C_r ^* (G)$ and $C^* (G)$ for general locally compact and Hausdorff topological groups.
  If a group $G$ is equipped with a discrete topology, then these notions coincide with the ones from the previous subsection.  

\begin{example}
  Let us present the Pontryagin duals of some locally compact Hausdorff abelian groups.
  \begin{itemize}
    \item $\widehat{\quot{\Z}{n\Z}} = \quot{\Z}{n\Z}$;
    \item $\widehat{\R} = \R$;
    \item $\widehat{\Z} = \mathbb{T}$;
    \item $\widehat{\mathbb{T}} = {\Z}$.
  \end{itemize}
  We notice that for each of these groups, the dual of a dual is the original group.
  This is not a coincidence.
\end{example}

\begin{theorem}[Pontryagin]
  If $G$ is a locally compact Hausdorff abelian group,
  then $G \approx \widehat{\widehat{G}}$.
\end{theorem}